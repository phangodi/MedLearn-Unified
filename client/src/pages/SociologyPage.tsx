      ...
      <div className="flex-1 flex flex-col relative z-10">
        ...
        <main className="flex-1 mx-auto px-6 lg:px-10 py-8 relative z-10 max-w-7xl">
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-12 max-w-4xl mx-auto">
            <div className="flex items-center justify-center gap-3 mb-4"><Users className="w-12 h-12 text-cyan-600 dark:text-cyan-400" /><h1 className="text-3xl sm:text-4xl lg:text-5xl font-extrabold bg-gradient-to-r from-foreground to-cyan-600 dark:to-cyan-400 bg-clip-text text-transparent">Sociology</h1></div>
            <p className="text-base lg:text-lg text-muted-foreground">Select an exam to begin studying</p>
          </motion.div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 max-w-4xl mx-auto">
            {exams.map((exam, idx) => (
              <motion.div key={exam.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.1 }} className={exam.enabled ? 'cursor-pointer' : 'cursor-not-allowed opacity-60'} onClick={() => exam.enabled && navigate(exam.path)}>
